...
final class PlanContract
{
    ...
    {
        return $this->record($actor, $proposal, null, ProposalActionType::CreateContract, $payload, null, $operationId, $expectedRevision);
    }
    ...
    private function record(User $actor, Proposal $proposal, ?ProposalItem $item, ProposalActionType $type, array $payload, ?string $reason, string $operationId, int $expectedRevision): ProposalAction
    {
        return DB::transaction(function () use ($actor, $proposal, $item, $type, $payload, $reason, $operationId, $expectedRevision): ProposalAction {
            ...
            if ($type === ProposalActionType::ChangeContractEconomics) {
                if ($item === null) {
                    throw ValidationException::withMessages(['item' => 'Elemento Contratto obbligatorio.']);
                }
                $candidate = ProposalItem::query()->where('proposal_id', $locked->id)->with('contract')->find($item->id);
                if ($candidate === null) {
                    throw ValidationException::withMessages(['item' => 'Elemento non disponibile.']);
                }
                $validated = ContractPlan::prepareEconomicChange($candidate, $validated, now($locked->company->timezone)->toDateString());
            }
            $previous = [];
            if ($item === null) {
                if ($type !== ProposalActionType::CreateContract) {
                    throw ValidationException::withMessages(['item' => 'Elemento Contratto obbligatorio.']);
                }
                $supplier = Supplier::query()->where('company_id', $locked->company_id)->whereNull('archived_at')->find($validated['supplier_id']);
                if ($supplier === null) {
                    throw ValidationException::withMessages(['supplier_id' => 'Fornitore attivo non disponibile in questa Azienda.']);
                }
                $lockedItem = $locked->items()->create(['proposal_item_id' => (string) Str::uuid(), 'company_id' => $locked->company_id, 'source_type' => ProposalSourceType::Contract, 'baseline' => ['plan_baseline' => [], 'actual_context' => ['has_actuals' => false]], 'result' => ContractPlan::create($validated), 'readiness_state' => 'aligned', 'readiness_reasons' => [], 'read_only_source' => false, 'last_aligned_at' => now(), 'last_aligned_by_id' => $actor->id]);
            } else {
