...
it('applies open years atomically while preserving and auditing a closed-year divergence', function (): void {
    $company = Company::factory()->create();
    $open2026 = Exercise::factory()->for($company)->create(['year' => 2026]);
    $closed2027 = Exercise::factory()->for($company)->create(['year' => 2027, 'status' => 'closed']);
    ...
    expect($retry->is($budget))->toBeTrue()
        ...
        ->and(Expense::query()->where('contract_id', $contract->id)->where('exercise_id', $open2028->id)->exists())->toBeTrue()
        ->and(collect($budget->affected_exercises)->firstWhere('exercise_id', $closed2027->id)['historical_divergence'])->toBeTrue()
        ->and(AuditEvent::query()->where('operation_id', $operationId)->where('event_type', AuditEventType::HistoricalDivergenceRecorded)->count())->toBe(1);
});
